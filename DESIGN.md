...
**Measured span = ~40.3M words (~320 MB), `over_align=False`** — **5× over the 2²³ (64 MB) default**, so the test asserts flat at a **raised limit (2²⁶)** per the §1.2 escape hatch (raise `--flat-max-words`, RAM-only cost). **⚠ SCALING FINDING (R-3):** the bloat is the **single COMBINED dispatch table with a 5-nibble index** — a 5-nibble index addresses a 16⁵-slot space, so one combined table of 198k texels costs ~40M words, **~7× `build_doom`'s narrow per-texture ledger (5.54M words for *more* texels, §1.2 above)**. The combined-table approach (chosen M12mm2+ to avoid a per-pixel texture re-select) trades that per-pixel hoisting for a *much* larger footprint that does **not** fit the 64 MB target and would balloon for the full game. **TODO / owner decision (future rung):** either (a) keep the combined table + raise the flat limit (RAM-only), or (b) switch to narrow **per-texture tables** with a per-column table pointer (set once/column in pass 1 — within the hoisting rule), recovering the ~7× and fitting 2²³. Correctness (byte-exact + golden) is independent of this choice. (build_doom wiring — folding the renderer into the shipped binary's ledger — is the immediate next rung and will surface this footprint in the R0 gate.)
