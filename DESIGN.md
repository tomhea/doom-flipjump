...
**Span ladder: pre-M12oo ~40.3M words (~320 MB) → M12oo ~31.2M → M12pp ~23.4M → M12qq ~21.8M words (~174 MB)**, `over_align=False` — still over the 2²³ (64 MB) default, so the test asserts flat at a **raised limit (2²⁶)** per the §1.2 escape hatch (raise `--flat-max-words`, RAM-only cost). **⚠ SCALING FINDING (R-3) — CORRECTED BISECTION (the combined table is NOT the dominant chunk, an earlier wrong guess):** the ~40.3M pre-M12oo span bisects (via `Reader(fjm).memory_segments` + assembling sub-parts) to **~21M the BSP WALK** (575 segs' baked per-seg/per-node `hex.set` constants — each pays an `@` dispatch to zero a register it overwrites — and `_bsp_as_code` emits each leaf's action **twice**) + **~16M the fully-unrolled 16K-pixel PASS-2 clip** + only **~3.5M the combined 198k-texel table** (measured standalone). So the per-texture-table idea would NOT help (~3.5M of 40M); the real levers are the walk and the pass-2 unroll. **M12oo (DONE)** attacked the pass-2 clip: it replaced the per-pixel inlined `hex.cmp`×2 (×16K) with a **shared-compare trampoline** (one `compare_y` body + a per-pixel `wflip` redirect), measured span **40.3M → 31.2M words (−9M / ~23%)**. **M12pp (DONE)** attacked the walk's per-SEG baked constants: it replaced the per-seg `hex.set` (each pays an `@` dispatch to zero a reg it overwrites) with `hex.xor_by` + **xor-involution self-zeroing** (a shared `seg{i}_xorby` block fcall'd before/after the leaf; `xor_by val` twice cancels, regs self-restore to 0), with the non-pure `worldtop` relocated INTO the leaf (`worldtop = seg_ceil − viewzw`, `seg_ceil` baked pure). Measured **31.2M → 23.4M words (−7.8M / ~25%)**, and the golden assemble **1090s → 262s** (the `@` dispatches were super-linearly expensive); byte-exact + golden hash preserved. **M12qq (DONE)** applied the SAME `xor_by` involution to the 681 BSP **NODE** partition consts (`cpx/cpy/cdx/cdy` in `_bsp_as_code`; SET/USE/CLEAR around the `point_on_side_leaf` fcall, cleared before recursion so children SET from a zero state): **23.4M → 21.8M words (−1.6M)**, order + golden byte-exact. *(The original M12qq plan — single-emission of subsectors — is **negligible post-M12pp**: the seg bodies are already shared `xorby` blocks, so the BSP double-emission only duplicates cheap fcalls. Measured-and-skipped.)* **build_doom WIRING (DONE):** the renderer is now extracted into the SHARED `doomfj.wall_renderer.emit_wall_renderer` (the SAME emitter the byte-exact golden test renders through — R6 single source), and `build.build_wall_renderer` ships it into one `.fjm` with the R0 gate (`storage_mode == flat`, `span < 2²⁶`; `test_build_wall_renderer_e1m1_flat`). So `build_doom`'s renderer path inherits every optimization (M12oo trampoline + M12pp/qq involution walk) for free. **Remaining (deferred optimizations, NOT blocking M13):** partition-const **width-shrink** (the original M12rr — *measured-and-skipped*: under `xor_by`, zero high-nibbles are already free, so only the negative sign-extended partition consts are shrinkable, ~<1M, and it would touch the R5-sensitive `point_on_side_leaf` cross product) + the pass-2 unroll (the C2 runtime row loop); then the ~3.5M table is the floor.
