...
> **@-sensitivity — the dominant budget variable (U7/R-6; the single most important R-1 measurement).** Every per-pixel line above is **@-proportional** (a dispatch is ~4@, a deposit ~4@/byte — §2 glossary), and **@ grows with total program size (U7)**. The lines are written at an implied **@≈15** (a small/medium build; back-solved from the inherited ~53–63 ops/byte and ~100–200 ops/px estimates). **At the §A "DOOM-scale" @≈27, the @-proportional work ~doubles** — the per-pixel reads+stores alone go from ~2.6–4.2M to ~**5–6M**, and the **total lands ~7–9M of 11.2M (margin ~1.3–1.5×, not 2×)**; near the @≈27 high end it can approach ~1.1×. So the headline "~2× margin" holds only at the smaller @. **R-1 (S5.3) measures the real @ at game scale before R2 commits**; costs are stated in **@** (scale-invariant) in the components and converted at the measured @. If @ lands high, the §2 fallbacks (flat-shaded, 12.5 fps, bpp=4) absorb it. *(This reconciles the §A @≈27 cost model with the inherited §2 ops-figures — formerly an un-converted @-vs-ops mismatch.)*
