...
**M12nn runtime-renderer (R-3 finding):** the capstone test `test_wall_render_e1m1_full_frame_golden` assembles the *actual runtime renderer* — the 681-node BSP-as-code walk + pass-1/**fully-unrolled 16K-pixel pass-2** + framebuffer + trig/colormap LUTs + a **198,337-texel combined wall-texture table** (only the 575 one-sided segs' textures, 2× downscaled, each slice pow2-aligned to its texheight) — and renders E1M1 byte-exact + the spawn golden. **Measured: this test layout's span EXCEEDS 2²³** (`storage_mode != flat` even at `flat_max_words = 2²³`), because it uses the leaf's `over_align=True` table placement (2ⁿ pad) **plus** the full per-pixel pass-2 unroll. That's a *non-production* footprint — the **shipped** binary's flat-span budget is enforced by `build_doom` (`build.py:35`, `storage_mode==flat` under 2²³) on the **hot-low / `over_align=False` production layout** (ledger total above: 5.54M < 2²³). **TODO (build_doom wiring rung):** assemble the renderer through `build_doom`'s production layout and re-assert the 2²³ flat budget end-to-end (fold the pass-2 shell + the 198k wall-table into the ledger).
