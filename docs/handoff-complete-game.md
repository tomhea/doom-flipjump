...
**Payoff -- MEASURED:** the host reload is gone. Alternating A/B on one harness, **3.85x on
CPU time** (1918.0 ms/frame -> 498.0 ms/frame). The in-program reset costs **270,811
ops/frame = 0.9% of the 30,191,585-op sweep MEDIAN** (an earlier 7.47% figure used the mean of
eight gate viewpoints -- the wrong denominator). 260/260 sweep frames byte-exact.
**M5 is unblocked.**
⚠ Those absolutes are HARNESS-bound, not the game's fps -- the ratio is the result.
⚠ **NOT WIRED YET.** The reset is generated by `scratchpad/m1_emit_reset.py` and assembled by
`scratchpad/m1_build.py`; `build.py` still emits the single-pass host-reset program and
`walk_e1m1.py` still points at it. That wiring is the remaining step before M1 counts as shipped.
