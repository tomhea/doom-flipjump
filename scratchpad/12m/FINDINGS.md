...
**`scratchpad/12m/mutcheck.py`** runs 18 mutations against the tests as they stand on disk. All
18 CAUGHT; all 13 touched sources restored byte-identical. Its own controls: C1 a mutation whose
anchor no longer matches is a BROKEN CHECK, never a silent skip; C2 every target file must be
green before anything is mutated; C3 every file is byte-compared after restore; C4 every
tests/host file this branch adds -- ASKED OF GIT, not listed, after a hardcoded twelve went on
reporting "all covered" while a thirteenth test file sat uncovered (CR-2026-09-16) -- must be
NAMED by some row, so neither a row retired later nor a test file added later can lose its FAIL
