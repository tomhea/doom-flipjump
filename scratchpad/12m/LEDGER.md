...
| D1 | M6 | KILLED. `distscale` LUT 8 -> 5 nibbles (values max 0x168EE) + `read_table 5` in plane_render. Correct and byte-exact, worth NOTHING | 14,884,585 .. 29,143,782 | bind 23,495,707 (**+2,027 vs S2**) | **NULL -- reverted.** Premise was a profiler artifact | 51,099,046 w (+3,074). m2_std_gate PASS. AY's "12.14% of the frame" was 100% misattributed: `distscale` is the LAST top-level label, so all following code was credited to it -- 0 of 9,726,044 ops were inside the table. See FINDINGS BA; hotpath now guards against it. |
| W1 | M6 | width narrowing: the two 8-nibble `sparse_mov 8, x, x1` calls -> `sparse_zero 8` + `sparse_mov 2` (x1 is a clipped column in [0,161], so identical final state). Found via the MATCHED-label profile | 14,908,548 .. 29,094,739 | **bind 23,447,960 (-0.19%)** | small WIN, byte-exact | 51,094,744 w (38.07%), -1,228. m2_std_gate PASS. ⚠ Predicted ~362k ops/frame, measured 45,720 -- **7.9x short**. Attributed ops are not removable ops (FINDINGS BC). |
