"""M13-EMIT rung 1: price the ~3.7k-ops [y2][colour] pair -- reads+loop vs the byte.emit dispatches."""
...
for tag, ab in (("full emit (shipped)          ", frozenset()),
                ("walk, no byte.emit pairs     ", frozenset({"emitnopair"})),
                ("no band walk at all          ", frozenset({"emitnowalk"}))):
    t0=time.time()
