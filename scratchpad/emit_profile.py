"""M13-EMIT rung 2: where do the frame's ops ACTUALLY go? (the shipped ablation ladder)"""
...
for tag, ab in (("walk skeleton only  (segstub)", frozenset({"segstub"})),
                ("+ per-seg entry     (xrstub) ", frozenset({"xrstub"})),
                ("+ the wedge test  (wedgestub)", frozenset({"wedgestub"}))):
    t0=time.time()
