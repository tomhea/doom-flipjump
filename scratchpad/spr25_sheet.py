...
ap.add_argument("--out", default="scratchpad/spr25_sheet.png")
...
for row, (vx, vy, va, name) in enumerate(VPS):
    ...
    print(f"  {name} ({vx},{vy},{va:#x})  left = all 251 things, right = 53 monsters only",
          flush=True)
out = ROOT / args.out
sheet.save(out)
print(f"\nwrote {out}   LEFT = every sprite (today) | RIGHT = the 25M package (monsters only)")
