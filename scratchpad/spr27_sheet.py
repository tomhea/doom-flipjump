...
ap.add_argument("--out", default="scratchpad/spr27_sheet.png")
...
for row, (vx, vy, va, name) in enumerate(VPS):
    ...
    print(f"  {name} ({vx},{vy},{va:#x})  left = all 251 things, right = the 27M set (81)",
          flush=True)
out = ROOT / args.out
sheet.save(out)
print(f"\nwrote {out}   LEFT = every sprite (251) | RIGHT = the 27M set (81: monsters + barrels + 6 props)")
