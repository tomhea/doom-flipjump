...
def _room():
    return compile_bsp(WadFile.from_path(ROOM), "MAP01")   # 256x256 square, verts (0,0)..(256,256)
...
def test_rw_distance_is_perpendicular():
    ...
    quantization; rw_normalangle = seg.angle_BAM - ANG90 (our winding convention)."""
    cmap = _room()
    rm = ReferenceModel()
...
def test_scale_perpendicular_centre_exact():
    ...
    0.625 exactly (all sines are 1.0 = 65536, no quantization)."""
    cmap = _room()
    rm = ReferenceModel()
    seg = cmap.segs[1]                                      # (256,0)->(256,256), angle 0x4000 -> normal 0
    nrm, rwd = rm.wall_setup(128 * U, 128 * U, seg, cmap.vertexes)
